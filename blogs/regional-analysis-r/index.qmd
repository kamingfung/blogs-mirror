---
title: Regional analysis in R
description: Attaching country and region labels to gridded data so it can be summarised by area.
date: 2026-04-10
categories:
- r
- geospatial
...
<div class="mh"><p class="mh-crumb"><a href="/series/earth-data/index.html">Earth Data in Python and R</a><span class="sep">·</span>Part 4 · Regions and masks</p><p class="mh-facts"><span class="lvl lvl-intermediate">Intermediate</span></p><p class="mh-assumes">Some programming in any language.</p></div>
<!-- CURRICULUM_HEADER_END -->

::: {.callout-note appearance="simple" icon=false}
## TL;DR

To summarise gridded data by country or state you need a region label on every (lon, lat) cell. In R, GADM polygons from `raster::getData` and a point-in-polygon test with `sp::over` attach that label; `tidyverse` then gives regional means with `group_by` and a regional map with `filter`. The example uses a random global field and US states.
:::

---

Here's a quick example of how to append geographic information to gridded data for regional analysis.
...
## Downloading US state boundaries
...
## Joining regions and aggregating

### Matching (lon, lat) pairs to polygons
...
- Give the matrix `dimnames` of lon and lat and `melt` it; a long (lon, lat, value) data frame is what both `ggplot2` and the spatial join want.
- GADM polygons give country and state boundaries at three levels of detail; `raster::getData` is retired in current versions of `raster`, and `geodata::gadm()` returns the same data.
- `sp::over` with `SpatialPoints` in the polygons' CRS returns the attribute row of the polygon each grid point falls in, or `NA` outside them.
- Join on the distinct (lon, lat) pairs rather than every row, so the point-in-polygon test runs once per grid cell and not once per time step.
- With region names attached, a regional mean is a `group_by` and `summarize`, and a regional map is a `filter`.

<!-- CURRICULUM_FOOTER_START -->
<div class="mf"><a class="mf-prev" href="/blogs/r-regridding/index.html"><span>Previous</span>Regridding crop data for climate models</a><a class="mf-up" href="/series/earth-data/index.html"><span>Series</span>Earth Data in Python and R</a><a class="mf-next" href="/blogs/ms-buildings/index.html"><span>Next</span>Downloading building footprints</a></div>
